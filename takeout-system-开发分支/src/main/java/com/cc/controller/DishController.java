...
public class DishController {
    ...
    public ResponseEntity<List<Dish>> getDishesWithLowStock() {
        List<Dish> dishes = dishRepository.findAll()
                .stream()
                .filter(dish -> dish.getQuantity() < dish.getAlertQuantity())
    ...
    public Result<List<DishDto>> listCategory(Dish dish){
        ...
        //如果没有，就根据查询数据库，再根据构造的Key存入一个商品数据
    ...
    public Result<String> delete(@RequestParam("ids") List<Long> ids){
        ...
        if(deleteInSetmeal){
            ...
            return Result.error("删除的商品中有关联在售套餐,删除失败！");
        }
    }
